...
class FRNet(BaseSequenceGenerator):
    ...
    def forward_sequence(self, lr_data):
        ...
        # pad if size is not a multiple of 8 (similar to inference path)
        pad_h = lr_flow.size(2) - lr_flow.size(2) // 8 * 8
        pad_w = lr_flow.size(3) - lr_flow.size(3) // 8 * 8
        if pad_h != 0 or pad_w != 0:
            lr_flow_padded = F.pad(lr_flow, (0, pad_w, 0, pad_h), "reflect")
        else:
            lr_flow_padded = lr_flow

        # upsample lr flows
        hr_flow = self.scale * self.upsample_func(lr_flow_padded)
        # crop back to original size
        hr_flow = hr_flow[..., :hr_h, :hr_w]
