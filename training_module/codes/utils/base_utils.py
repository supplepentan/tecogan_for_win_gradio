...
def parse_agrs():
    ...
    parser.add_argument(
        "--opt", type=str, required=True, help="path to the config yaml file"
    )
    parser.add_argument(
        "--gpu_ids", type=str, default="-1", help="GPU index (set -1 to use CPU)"
    )
    parser.add_argument(
        "--lr_size", type=str, default="3x256x256", help="size of the input frame"
    )
    parser.add_argument(
        "--test_speed", action="store_true", help="whether to test speed"
    )
    parser.add_argument("--local_rank", default=-1, type=int, help="local gpu index")
    return parser.parse_args()


def parse_configs(args):
    # load option file
    with open(osp.join(args.exp_dir, args.opt), "r", encoding="utf-8") as f:
