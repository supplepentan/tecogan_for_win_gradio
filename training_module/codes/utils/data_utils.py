...
from scipy import signal  # Keep this for other signal processing functions if any
from scipy.signal import windows  # Import the windows module
import cv2
import numpy as np
import torch
...
def create_kernel(sigma, ksize=None):
    ...
    gkern1d = windows.gaussian(ksize, std=sigma).reshape(
        ksize, 1
    )  # Changed from signal.gaussian
